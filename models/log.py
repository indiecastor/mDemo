...
def model(y: float, t: any) -> float:
    return log_growth_speed * y * (1 - (y / log_capacity))
...
def drag_line(param: Literal['log_capacity', 'log_init_pop']):
    ...
    match param:
        case 'log_capacity':
            if value >= LOG_CAPACITY_MAX:
                dpg.configure_item('log_capacity_line', default_value=LOG_CAPACITY_MAX)
                log_capacity = LOG_CAPACITY_MAX
            elif value <= LOG_CAPACITY_MIN:
                dpg.configure_item('log_capacity_line', default_value=LOG_INIT_POP_MIN)
                log_capacity = LOG_CAPACITY_MIN
            else:
                log_capacity = value
        case 'log_init_pop':
            if value >= LOG_INIT_POP_MAX:
                dpg.configure_item('log_init_pop_line', default_value=LOG_INIT_POP_MAX)
                log_init_pop = LOG_INIT_POP_MAX
            elif value <= LOG_INIT_POP_MIN:
                dpg.configure_item('log_init_pop_line', default_value=LOG_INIT_POP_MIN)
                log_init_pop = LOG_INIT_POP_MIN
            else:
                log_init_pop = value
